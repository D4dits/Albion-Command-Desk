...
def test_stream_snapshots_keeps_trusted_party_member_loot(monkeypatch) -> None:
    party = PartyRegistry()
    party.seed_names(["PartyOne"])
    loot_tracker = LootTracker(
        party_registry=party,
    ...
    events = loot_tracker.events()
    assert len(events) == 1
    assert events[0].looted_by.player_name == "RosterNotResolvedYet"
