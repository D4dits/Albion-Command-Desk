...
def validate_setup(setup: CraftSetup) -> list[str]:
    ...
    if setup.station_fee_percent < 1 or setup.station_fee_percent > 999:
        errors.append("station_fee_percent must be in range 1..999")
...
def sanitized_setup(setup: CraftSetup) -> CraftSetup:
    ...
    return CraftSetup(
        ...
        station_fee_percent=clamp(setup.station_fee_percent, 1.0, 999.0),
